...
const formatDataJson = (data): TypeJsonFormat[] => {
  return data.map((item: ListData) => {
    return {
      ...
      phone: item?.phone,
      history_sicknes: item?.history_sicknes,
      photo: item?.photo,
      role: item?.role,
...
export default function ControlDiabetesTable({ data = [] }: { data: any[] }) {
  ...
  const editStatus = (id: string, status: boolean) => {
    try {
      httpRequest({
        url: '/user',
        method: 'put',
  ...
  const onChangeStatus = useCallback((id: string, status: boolean) => {
